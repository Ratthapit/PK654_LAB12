...
int findYugiAction(int s){	
	if(s == 9) return 2; // Yugi will definitely stay (2) when current score (s) is equal to 9
		 ...
		 int YugiDecide =rand()%100 + 1;// If current score is 6,7,8, Yugi will draw with probability 69% and will stay with probability 31% 
		 if(YugiDecide>31) return 1;
		 else  return 2;
	}
}
